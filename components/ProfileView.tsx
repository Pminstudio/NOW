...
import { MOCK_PULSES } from '../constants';
...
const ProfileView: React.FC<ProfileViewProps> = ({ profile, isCurrentUser, onBack, onPulseClick, onReservationsClick }) => {
  const isPulseur = 'rating' in profile;
  const activePulses = MOCK_PULSES.filter(p => p.pulseurId === profile.id);

  // Urban/Lifestyle adult backgrounds
  const profileBg = isCurrentUser 
    ? "https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=1200&q=80" // Modern urban social
    : "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=1200&q=80"; // Concert/Event adult scene
  ...
  return (
    ...
    >
      <div className="relative h-80 w-full overflow-hidden">
        ...
        <button 
          onClick={onBack} 
          className="absolute top-10 left-10 w-16 h-16 bg-white/20 backdrop-blur-2xl border border-white/30 rounded-[28px] flex items-center justify-center transition-all active:scale-90 text-white shadow-2xl"
        >
      ...
      <div className="px-10 mt-28 space-y-16 pb-24">
              ...
              {activePulses.map(pulse => (
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    ...
                    <p className="text-xs text-gray-400 font-black uppercase tracking-tighter">Maintenant • Marseille</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-violet-950 p-12 rounded-[55px] text-center shadow-2xl shadow-violet-900/30">
            <p className="text-6xl font-black text-white mb-2 italic leading-none">{isCurrentUser ? '12' : '42'}</p>
        ...
        <div className="mt-12">
          {isCurrentUser ? (
            <button className="w-full py-8 bg-red-50 text-red-500 rounded-[45px] text-2xl font-black transition-all active:scale-95 border-2 border-red-100 uppercase tracking-tighter italic">
              Se déconnecter
            </button>
          ) : (
            <button className="w-full py-8 bg-violet-950 text-white rounded-[45px] text-3xl font-black shadow-4xl transition-all active:scale-95 transform hover:bg-black uppercase tracking-tighter italic">
              Envoyer un Pulse
            </button>
          )}
        </div>

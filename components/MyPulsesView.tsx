...
interface MyPulsesViewProps {
  ...
  onRemoveFavorite?: (pulseId: string) => void;
}
...
const MyPulsesView: React.FC<MyPulsesViewProps> = ({
  ...
  const now = new Date();
  const upcomingPulses = reservations.filter(p => new Date(p.startTime) >= now);
  const pastPulses = reservations.filter(p => new Date(p.startTime) < now);

  const displayedPulses = activeTab === 'upcoming'
    ? upcomingPulses
    : activeTab === 'past'
      ? pastPulses
      : favorites;
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-8 pt-8 pb-32 scrollbar-hide">
        <div className="mb-12">
          ...
          <div className="mt-8 flex gap-2 p-1.5 bg-violet-50 rounded-[24px] border border-violet-100/50">
            <button
              onClick={() => setActiveTab('upcoming')}
              className={`flex-1 py-3 rounded-[18px] text-[9px] font-black uppercase tracking-wider transition-all ${activeTab === 'upcoming' ? 'bg-white text-violet-950 shadow-lg' : 'text-violet-300'}`}
            >
              À Venir ({upcomingPulses.length})
            </button>
            <button
              onClick={() => setActiveTab('favorites')}
              className={`flex-1 py-3 rounded-[18px] text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 ${activeTab === 'favorites' ? 'bg-white text-violet-950 shadow-lg' : 'text-violet-300'}`}
            >
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-[18px] text-[9px] font-black uppercase tracking-wider transition-all ${activeTab === 'past' ? 'bg-white text-violet-950 shadow-lg' : 'text-violet-300'}`}
            >
              Passé
            </button>
          </div>
            ...
            {displayedPulses.length === 0 ? (
              ...
              >
                ...
                <p className="text-violet-950 font-black uppercase text-[12px] tracking-widest mb-2 italic">
                  {activeTab === 'favorites' ? 'Pas encore de favoris' : 'Rien de prévu ici'}
                </p>
                <p className="text-violet-300 text-[10px] font-bold uppercase tracking-widest mb-8">
                  {activeTab === 'favorites'
                    ? 'Ajoute des pulses en favoris pour les retrouver facilement.'
                    : 'Ne laisse pas le temps défiler sans pulser.'}
                </p>
                <button
                  onClick={onBack}
                  ...
                  <div className="flex p-6 gap-6 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {activeTab === 'favorites' && (
                          <span className="bg-rose-100 text-rose-500 px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest flex items-center gap-1">
                    ...
                    {activeTab === 'favorites' && onRemoveFavorite && (
                      <button
                        onClick={() => onRemoveFavorite(pulse.id)}
                        className="w-14 h-14 bg-rose-50 text-rose-400 rounded-2xl flex items-center justify-center active:scale-90 transition-all hover:bg-rose-500 hover:text-white"
